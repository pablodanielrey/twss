...
if __name__ == '__main__':
    ...
    with open('data/links.ttl','w') as f:
        for st, sp, so in g.triples((None, RDF.type, schema.Person)):
            print(f'Buscando datos de  {st}')
            for stt, spp, soo in g.triples((st, schema.name, None)):
                name = quote(str(soo).replace(' ','_'))
                break
            else:
                print('La persona no tiene nombre!!!')
                continue    
            
            r = requests.get(f"https://dbpedia.org/data/{name}.ttl")
            if not r.ok:
                print(f'No se pudo obtener info de dbpedia de {name}')
                print(r.text)
                sys.exit(1)
                continue
            ...
            gactor = Graph()
            turtle = r.text
            gactor.parse(data=turtle, format='turtle')
            print('Imprimiendo tripletas')
            propiedades = [
                dbp.birthDate,
                dbp.occupation,
                foaf.isPrimaryTopicOf
            ...
            for p in propiedades:
                for stt, spp, soo in gactor.triples((None, p, None)):
                    print(stt, spp, soo)
            sys.exit(1)